...
router.route('/getRiddle').get(async (req, res) => {
  ...
  try {
      ...
      return res.status(200).json({riddle_text,answer});
  } catch (error) {
...
router.route('/checkAnswer').post(async (req, res) => {
  ...
  try {
      ...
      if (isCorrect) {
          return res.status(200).json(riddle.correct_scene);
      } else {
          return res.status(400).json(riddle.incorrect_scene);
      }
  } catch (error) {
      res.status(400).json('Error: ' + error);
  }
